...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, original_df: pd.DataFrame) -> Dict[str, Any]:
    """
    ...
    flags["has_duplicate_columns"] = is_equal(original_df) # есть и дубликаты колонок
    flags["has_constant_columns"] = is_constant(original_df) # в колонке только одно уникальное значение
    flags["has_high_cardinality_categoricals"] = has_high_cardinality_categoricals(original_df) # много уникальных значений в категориальных признаках
...
def is_equal(df: pd.DataFrame) -> bool:
    """
    Проверяет, есть ли в DataFrame хотя бы две полностью одинаковые колонки
    """
...
def is_constant(df: pd.DataFrame) -> bool:
    """
    Проверяет, есть ли в DataFrame хотя бы одна колонка с одним значением
    """
    columns = df.columns.to_list()
    for i in columns:
        if (len(df[i].unique()) == 1): return True
    return False

def has_high_cardinality_categoricals(df: pd.DataFrame) -> bool:
    """
    Проверяет, есть ли категориальные признаки с очень большим числом уникальных значений
    """

    for i in df.columns:
        col = df[i]
        if (pd.api.types.is_categorical_dtype(col) or
            pd.api.types.is_string_dtype(col) or
            pd.api.types.is_object_dtype(col)):

            n_unigue = col.nunique(dropna=True)
            if n_unigue / len(col) > 0.7:
                return True
